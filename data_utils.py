...
# Modified by NJ Ref: https://github.com/scikit-image/scikit-image/issues/4241
# import skimage
...
class Dataset(object):
    ...
    def __init__(self, scan_files, seg_files):
        ...
        """
        # print('NJ1111111:', len(scan_files), len(seg_files))
    ...
    def build_dataset(self):
        ...
        segs = {pathlib.PureWindowsPath(seg).as_posix().split('/')[-1]: nrrd.read(seg)[0] for seg in self.seg_files} #NJ: Note this is a dictionary
        #print(segs.keys())
        ...
        for i, scan in enumerate(scans):
            ...
            if scan.PatientID not in patients:
                ...
                # print('NJ22222222, pat_folder_regex:', pat_folder_regex)
                # print('NJ22222222, scan_files[i]:', self.scan_files[i])

                # Added by NJ
                p = pathlib.PureWindowsPath(self.scan_files[i])
                p_str = p.as_posix()


                patient_folder = re.search(
                    #pat_folder_regex, self.scan_files[i]).group(1)
                    pat_folder_regex, p_str).group(1)

                # temp_scan_file = './data/raw/train\Prostate-3T\Prostate3T-01-0001\03-15-2003-NA-MR prostaat kanker detectiemc MCAPRODET-79232\2.000000-t2tsetra-49698\1-01.dcm'
                # print('NJ3333333333, patient_folder:', patient_folder)

                seg_file = segs['%s.nrrd' % patient_folder]

                # print('NJ4444444, seg_file:', seg_file)
    ...
    def load_dataset(path):
        ...
        print('Path: in data_utils.py', path)
...
if __name__=='__main__':
    ...
    #Section Added by NJ for testing

    from glob import glob

    print('in data_utils.py file!!!')
    ...
    train_seg_files = glob('../data/raw_nrrd/train/**/*.nrrd', recursive=True)
    # train_seg_files += glob('./data/raw_nrrd/leaderboard/**/*.nrrd', recursive=True)
    # test_seg_files = glob('./data/raw_nrrd/test/**/*.nrrd', recursive=True)
